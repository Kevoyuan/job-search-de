---
name: job-search-de
description: Build a configurable candidate profile from supplied CVs and a few choices, then discover, verify, evidence-score, report, and synchronize German AI/ML jobs. Use for requests such as updating a search, finding German AI roles, rerunning job discovery, producing job reports, or maintaining the job-search Workbench.
---

# job-search-de — Configurable German Job-Search Pipeline

This skill is a search and evaluation engine. It does not store any candidate-specific profile inside the skill directory. Each user or project keeps its background and preferences under `.job-search/` in the active working directory.
